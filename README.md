...
- 提及 token 为扁平化形态（`/`、`.` → `-`），取最短无歧义后缀（文件取父目录段+名字、目录取名字，冲突时向上补段）；chip 文案显示该 token 本身（内置装饰的固定行为）。
