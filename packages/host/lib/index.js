...
*/
function rankRows(rows, query, limit = 20) {
	const q = normalizeQuery(query);
	const scored = [];
	...
	scored.sort((a, b) => a.score - b.score || a.row.path.length - b.row.path.length || a.row.path.localeCompare(b.row.path));
...
const DEFAULT_INDEX_LIMIT = 5e3;
const DEFAULT_INDEX_DEPTH = 14;
const DEFAULT_INDEX_CACHE_ENTRIES = 32;
const DEFAULT_INDEX_TTL_MS = 15e3;
const DEFAULT_SEARCH_INDEX_LIMIT = 1e5;
...
const DEFAULT_FILE_SAFE_SIZE_BYTES = 409600;
const DEFAULT_FILE_TEXT_LIMIT = 6e4;
/** @dir: tree snapshot budget. */
const DEFAULT_DIR_TREE_DEPTH = 3;
const DEFAULT_DIR_TREE_LINES = 200;
const DEFAULT_DIR_FILE_SIZE_BYTES = 32768;
const DEFAULT_DIR_FILE_TEXT_LIMIT = 24e3;
const DEFAULT_DIR_FILE_COUNT = 8;
const DEFAULT_DIR_MESSAGE_LIMIT = 6e4;
const DEFAULT_MAX_CONTEXT_TOKENS = 12e3;
...
function suffixFlattenTokens(path) {
	...
	const out = [];
	for (let k = 1; k <= segments.length; k++) out.push(flattenPath(segments.slice(-k).join("/")));
	return out;
}
/** Workspace-relative display path for a directly-resolved token. */
function relFor(cwd, token) {
	const value = isAbsolute(token) ? relative(cwd, token) : token;
...
function isMutationActor(actor) {
	...
	return operation === "write" || operation === "edit" || operation === "tool:write" || operation === "tool:edit";
}
/** Format the `<file_context>` message text (handoff template). */
function fileContextText(rel, read, limit) {
	const content = read.truncated ? `${read.text}\n… [truncated at ${limit} characters]` : read.text;
	return [
		"<file_context>",
...
let FileIndexService = (() => {
	...
	return class FileIndexService extends _classSuper {
		...
		indexFlights = /* @__PURE__ */ new Map();
		searchCache = /* @__PURE__ */ new Map();
		searchFlights = /* @__PURE__ */ new Map();
		queryCache = /* @__PURE__ */ new Map();
		queryFlights = /* @__PURE__ */ new Map();
		/** Prevent an in-flight pre-mutation walk from repopulating the cache. */
		indexGenerations = /* @__PURE__ */ new Map();
		...
		*/
		...
		invalidateIndex(cwd) {
			const keys = cwd === void 0 ? /* @__PURE__ */ new Set([
				...this.indexCache.keys(),
				...this.indexFlights.keys(),
				...this.searchCache.keys(),
				...this.searchFlights.keys(),
				...[...this.queryCache.keys(), ...this.queryFlights.keys()].map((key) => key.slice(0, key.indexOf("\0")))
			]) : /* @__PURE__ */ new Set([cwd]);
			if (cwd === void 0) {
		...
		deleteQueryEntries(cwd) {
			...
			for (const key of this.queryFlights.keys()) if (key.startsWith(prefix)) this.queryFlights.delete(key);
		}
		...
		ensureQueryByCwd(cwd, query) {
			...
			const build = this.ensureSearchCatalogByCwd(cwd).then((catalog) => {
				if ((this.indexGenerations.get(cwd) ?? 0) !== generation) return this.ensureQueryByCwd(cwd, query);
				const snapshot = {
			...
			while (head < queue.length && rows.length < this.config.indexLimit) {
				...
				for (const entry of entries) {
					...
					if (entry.type === "directory") {
						...
						if (depth < this.config.indexDepth) queue.push({
							target: entry.target,
							rel: path,
							depth: depth + 1
						});
					} else if (entry.type === "file") rows.push({
						type: "file",
						path,
						name: entry.name,
						dir: rel
					...
					for (const reference of scanReferences(block.text)) {
						...
						for (const hit of hits) {
							...
							if (estimatedTokens > remainingTokens) {
								this.warn(`skipping referenced ${hit.kind} "${hit.path}" because its context estimate (${estimatedTokens} tokens) exceeds the remaining budget (${remainingTokens} tokens)`, "context budget");
								continue;
							}
		...
		*/
		async resolveRefs(cwd, token, signal, exact = false) {
			...
			try {
				...
				const target = await this.ctx.fs.resolve(token, {
					cwd,
					signal
				});
				if (!this.ctx.fs.contains(root, target)) return [];
				const info = await this.ctx.fs.stat(target, signal);
				if (info !== void 0 && (info.type === "file" || info.type === "directory")) {
			...
			if (signal.aborted) return [];
			const dirRows = rows.filter((row) => row.type === "directory" && (row.path === norm || row.path.endsWith(`/${norm}`)));
			if (dirIntent) {
			...
			const fileRows = rows.filter((row) => row.type === "file" && (row.path === norm || row.path.endsWith(`/${norm}`)));
			const total = fileRows.length + dirRows.length;
			if (total >= 1 && total <= 2) return [...fileRows.map((row) => ({
				kind: "file",
				path: row.path
			})), ...dirRows.map((row) => ({
				kind: "directory",
		...
		async buildRefMessage(hit, cwd, signal, charLimit) {
			try {
				const text = hit.kind === "file" ? await this.buildFileText(hit, cwd, signal, charLimit) : await this.buildDirText(hit, cwd, signal, charLimit);
		...
		async buildFileText(hit, cwd, signal, charLimit) {
			const target = hit.target ?? await this.ctx.fs.resolve(hit.path, {
				cwd,
				signal
			});
			if (!await this.isInsideWorkspace(cwd, target, signal)) return void 0;
			const info = await this.ctx.fs.stat(target, signal);
		...
		async readFileText(target, info, signal, limit) {
			if (info.size !== void 0 && info.size <= this.config.fileSafeSizeBytes) return bounded(await this.ctx.fs.readText(target, signal), limit);
			let out = "";
		...
		*/
		async buildDirText(hit, cwd, signal, charLimit) {
			...
			const root = await this.ctx.fs.resolve(cwd, { signal });
			if (!this.ctx.fs.contains(root, target)) return void 0;
			const info = await this.ctx.fs.stat(target, signal);
			if (info === void 0 || info.type !== "directory") return void 0;
			...
			for (const candidate of contentCandidates) {
				...
				if (candidate.size !== void 0 && candidate.size > this.config.dirFileSizeBytes) continue;
				let text;
				try {
				...
				sections.push({
					rel: candidate.rel,
					read: bounded(text, this.config.dirFileTextLimit)
				});
			}
			if (signal.aborted) return void 0;
			...
			body += `\n${sectionsText.join("\n")}`;
			if (body.length > messageLimit) body = `${body.slice(0, Math.max(0, messageLimit - 32))}
… [directory context truncated]`;
		...
		estimateMessageTokens(message) {
			...
			if (meter?.estimateMessage !== void 0) try {
		...
		turnBudget(agentId, turn) {
			...
			if (existing !== void 0 && existing.turn === turn) return existing;
			const state = {
				turn,
			...
			if (this.turnState.size > 64) for (const key of this.turnState.keys()) {
...
//#endregion
export { Config, decodeFileReference, FileIndexService as default, name, scanReferences, scanTokens };
